...
#-------------------------------------------------metodologia---------------------------------------------------------------------------------
    ...
    if proceso!="Presencial Caja":
        Fichatecnica=workbook.add_worksheet("Descripción Ficha Técnica")
        Fichatecnica.merge_range(9,2,9,5,'FICHA TÉCNICA',workbook.add_format({'bold': True,'align': 'center','valign':'vcenter','text_wrap':True,'left':2,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#D3D3D3'}))
        Fichatecnica.merge_range(10,2,10,3,'Concepto',workbook.add_format({'bold': True,'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#D3D3D3'}))
        Fichatecnica.write(10,4,'Definición',workbook.add_format({'bold': True,'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#D3D3D3'}))
        Fichatecnica.write(10,5,'Interpretación en este informe',workbook.add_format({'bold': True,'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#D3D3D3'}))
        conceptos = ["Población","Tamaño muestra estimada","Muestra alcanzada","Varianza estimada","Error estimado","Nivel de confiabilidad","Observaciones"]
        for i, concepto in enumerate(conceptos, start=1):
            Fichatecnica.merge_range(10+i,2,10+i,3,f'{concepto}',workbook.add_format({'bold': True,'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#FFFFFF'}))
        descripciones = [
        "Comprende a TODOS los elementos (funcionarios, estudiantes, docentes y egresados, entre otros) que serán objeto de estudio o que utilizaron el servicio y deberán diligenciar el cuestionario",
        "Subconjunto estimado, estadísticamente, de la población que debe diligenciar el cuestionario ante la imposibilidad de encuestar a todos los elementos de la misma",
        "Subconjunto de la población que en efecto diligenció el cuestionario durante la etapa de recolección de información o trabajo de campo",
        "Promedio de la variabilidad (dispersión respecto a la media) de los resultados de cada uno de los atributos a evaluar",
        "Hace referencia a la desviación del dato de interés calculado a partir de la muestra con respecto a su respectivo parámetro poblacional; es decir que considerando los valores estándar, con un error del 5%, el dato estimado para la población podrá estar entre un -5% y un +5% de la estimación.",
        "Expresa la certeza de que el valor estimado se encuentre dentro del margen de error; considerando los valores estándar, si obtenemos un nivel de confianza del 95% quiere decir que si repitiese 100 veces la encuesta, 95 veces tendría resultados similares y el dato estimado estaría dentro del error y 5 veces fuera."]
        for k, descripcion in enumerate(descripciones, start=1):
            Fichatecnica.write(10+k,4,f'{descripcion}',workbook.add_format({'align': 'left','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#ffffff'}))

        formulas_excel = [
        '=IF(\'T+G\'!G9="-","Se desconoce el número de personas que emplearon el servicio","En total "&\'T+G\'!G10&" personas utilizaron el servicio y serán objeto de estudio")',
        # Tamaño de muestra
        "=IF('T+G'!G9=\"-\",\"En este caso particular no fue posible calcular el tamaño de la muestra, dado que no se tiene conocimiento acerca del tamaño de la población\",\"De acuerdo con la estimación de la muestra, a partir de la fórmula estadística estándar para este fin, se deben encuestar \"&'T+G'!G10&\" personas\")",

        # Muestra alcanzada
        "='T+G'!G11 & \" personas diligenciaron el cuestionario\"",

        # Varianza estimada
        "=\"La varianza estimada fue del \" & ROUND('T+G'!G12*100,1) & \"%\"",

        # Error estimado
        "=IF('T+G'!G13=\"-\","
        "\"No es posible hallar el error estimado dado que se desconoce el tamaño muestral y el poblacional\","
        "\"Si extrapolamos los resultados, con un error del \" & ROUND('T+G'!G13*100,1) & \"% esperamos que el valor estimado del nivel de satisfacción para la población se encuentre en un rango de \" & ROUND(J11,2) & \" y \" & ROUND(J12,2))",

        # Nivel de confiabilidad
        "=IF('T+G'!G14=\"-\","
        "\"No es posible determinar el nivel de confiabilidad dado que no se conoce el tamaño muestral ni el poblacional\","
        "\"Si se repitiese el experimiento 100 veces, en \" & ROUND('T+G'!G14,0) & \" ocasiones el valor estimado para la población caería dentro del intervalo identificado a partir del error estimado\")"]
        for l, formula in enumerate(formulas_excel, start=1):
            Fichatecnica.write(10+l,5,f'{formula}',workbook.add_format({'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#ffffff'}))
        
        Fichatecnica.merge_range(17,4,17,5,'Lo ideal es que la muestra alcanzada siempre sea igual o superior al tamaño de la muestra estimada, así se puede asegurar la representatividad para toda la población, es decir, a partir de los resultados obtenidos se pueden hacer inferencias de toda la población. Si esto no sucede, simplemente se debe mencionar que los resultados solo harán referencia a la muestra encuestada y que éstos no puede extrapolarse a la población.',workbook.add_format({'align': 'left','valign':'vcenter','text_wrap':True,'left':1,'right':1,'top':1,'bottom':1,'border_color':'black','bg_color':'#ffffff'}))
        Fichatecnica.set_column('E:E', 60)
        Fichatecnica.set_column('F:F', 55)
        Fichatecnica.set_row(17,70)
            ...
            for idx, col_filtro in enumerate(columnas_filtros_dinamicos):
                ...
                if col_filtro in data.columns.tolist():
                    try:
                        ...
                        # Add2 es compatible con Excel 2013+ para slicers de tabla (ListObject)
                        try:
