def excel_exportar(data, nombre_archivo,numerodepoblacion, Preguntas,columnas_observaciones,general,oficina,proceso, perido,tipos_grafica, columnas_filtros_dinamicos=[]):
    ...
    labels_graph = [ "Muy satisfactorio", "4","3","2","Muy malo","No aplica"]
...
#-------------------------------------------------insertar valores ficha varianza----------------------------------------------------------------------------------
    ...
    for row in range(8, 14):
        if row==13:
            ...
            TG.merge_range(row, 6, row, 7, '=IFERROR((1 - (((G9 - G11) / G9)^0.5) * ((G12 / (G11 - 1))^0.5)) * 100, 0)', workbook.add_format({'align': 'center','left':1,'right':2,'bottom':2,'bg_color': '#FFFFFF','border_color': 'black','num_format': '0'}))
        elif row==12:
            ...
            TG.merge_range(row, 6, row, 7, '=1-G14%', workbook.add_format({'align': 'center','right':2,'left':1,'bottom':1,'bg_color': '#FFFFFF','border_color': 'black', 'num_format': '0%'}))
        elif row==11:
            TG.merge_range(row, 4, row, 5, titulos_fichas[row-8], workbook.add_format({'align': 'center','left':2,'bottom':1,'bg_color': '#FFFFFF','border_color': 'black'}))
